...
standardize_factor <- function(df) {
  df %>%
    mutate(DisplayScore = if_else(class(DisplayScore == "numeric"),
      if_else(DisplayScore > 0, "positive",
        "negative"
      ),
      DisplayScore
    )) %>%
    mutate(
      DisplayScore = str_replace_all(DisplayScore, "[G|g]ood", "positive"),
      DisplayScore = str_replace_all(DisplayScore, "[G|g]ood", "negative")
    )
}
...
theme_fig_base_core <-
  theme_pubr(
    base_size = 52 / fig_anova_scale,
    legend = "right"
  )

theme_fig_base_scale <-
  c(
    scale_color_manual(values = cbPalette),
    scale_fill_manual(values = cbPalette),
    scale_alpha_manual(values = c(Good = 1, Bad = 0, good = 1, bad = 0, positive = 1, negative = 0, `1` = 0.2, `2` = 0.5, `3` = 0.8, `4` = 1))
  )
theme_fig_base <- list(
  theme_fig_base_core,
  theme_fig_base_scale
...
is_score_positive <- function(score) {
  # score is a vector
  # score can be either numeric or character
  if (class(score) == "numeric") {
    return(score > 0)
  } else {
    # validate the score only contains "positive" or "negative"
    if (all(score %in% c("positive", "negative"))) {
      return(score == "positive")
    } else {
      stop("score should be either numeric or character with 'positive' or 'negative'")
    }
  }
}
...
summarize_performance <- function(data, x_var, y_var, group, id = PlayerID) {
  ...
  data %>%
    group_by({{ id }}, !!enquo_x_var, !!enquo_group) %>%
    summarise(
      !!mean_name := mean(!!enquo_y_var, na.rm = TRUE),
      count = n()
    ) %>%
    ungroup() %>%
    tidyr::complete({{ id }}, !!enquo_x_var, !!enquo_group, fill = setNames(list(NA), mean_name)) %>%
    inner_join(data %>% select({{ id }}) %>% unique())
}
...
do_anova <- function(processed_data, factorA, factorB, value_tobe_summarised, tech = TRUE) {
  ...
  anovakun(data, "sAB", long = TRUE, tech = tech)
}
...
gg_filled_box <- function(box_width = 0.75, maxwidth = 0.5, scale = "width", dodge_width = 0.8) {
  list(
    geom_boxplot(
      aes(
        color = after_scale(
...
gg_box_sina <- function(box_width = 0.75, maxwidth = 0.5, scale = "width", dodge_width = 0.8) {
  list(
    geom_boxplot(
      width = box_width,
      outlier.shape = NA
...
test_func <- function(x, y) {
  paste0(rlang::quo_name(enquo(x)), "~", rlang::quo_name(enquo(y)))
}
...
summary_posthoc_test2 <- function(df_rule_hit, target, factor1, factor2) {
  ...
  if (class(df_rule_hit$DisplayScore) == "numeric") {
    df_rule_hit <- df_rule_hit %>% mutate(DisplayScore = if_else(DisplayScore > 0, "positive", "negative"))
  }
  df_rule_hit %>%
    mutate(PlayerID = as.factor(PlayerID)) %>%
    mutate(Correct = as.numeric(Correct)) %>%
