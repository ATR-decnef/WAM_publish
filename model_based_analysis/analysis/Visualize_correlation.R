...
visualize_correlation = function(file_name, fig_scale = fig_anova_scale, corr_method = "pearson"){
  ...
  num_of_params = data_df %>% select(-PlayerID, -iteration) %>% colnames %>% length() %>%  `/`(2)
  print(num_of_params)
  stopifnot(num_of_params %% 1 == 0)
  
  corr_df = data_df %>% select(-PlayerID, -iteration) %>% cor(method = corr_method) %>% `[`(1 : (num_of_params), (2*num_of_params) : (num_of_params + 1)) %T>% View() 
  # Get p-value matrix
  ...
  p_density_list =
    ...
    (data_df %>%
      ggplot(data = . , aes(x = .data[[param]])) + geom_density() + labs(title = fig_title) +
       theme_minimal())
      # ggsave(sprintf("%s_dense_%s.pdf", file_name_no_dot, param), plot = ., width = 100, height = 100, units = "mm")
  }
  
  names(p_density_list) = data_df  %>% select(-PlayerID, -iteration) %>% colnames
  cor_plot_labs
}
...
scatter_correlation = function(file_name, fig_scale = fig_anova_scale){
  ...
  scatter_param_list =
    ...
    colnames %>%
    `[` (!data_df  %>% select(-PlayerID, -iteration) %>% colnames %>% str_detect("res_"))
  ...
  foreach(param = scatter_param_list) %do%{
    ...
    p_alpha_recovery_scatter =
      ...
      theme_fig_base + xlab(expression("true ")) +
...
res = foreach(filename = file_lists, 
              .combine = list, 
