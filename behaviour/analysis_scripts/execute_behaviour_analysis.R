...
cbPalette <- c(edge = "#E69F00", center = "#56B4E9", 
...
## ----load_data----------------------------------------------------------------
df_rule_hit_origin <- read_csv(here::here("data/df_rule_hit_untilMar_switch.csv")) %>% mutate(PlayerID = as.factor(PlayerID))%>%
  mutate(EstRule = str_replace_all(EstRule, pattern = c("luck"="random")),
         TrueRule = str_replace_all(TrueRule, pattern = c("luck"="random"))) %>% 
  mutate(TrueScore = if_else(TrueScore < 0, 0, TrueScore),
...
playerid_list = df_rule_hit %>% pull(PlayerID) %>% as.factor() %>% unique()
...
df_rle%>% 
  select(-data, -rle)%>% 
  mutate(length_of_block = lengths,
...
general_acc_included = df_rule_hit_origin %>% 
    ...
    pivot_wider(names_from = is_included, values_from = accuracy) %>% summarise(accuracy = mean(included, na.rm = TRUE)) %>% pull(accuracy)
num_of_all_participants = df_rule_hit_origin %>% pull(PlayerID) %>% unique() %>% length()
...
p_general_acc = df_rule_hit_origin %>% 
   ...
   ggplot(aes(x = "", y = accuracy)) + 
     ...
     theme_fig_boxplot + xlab("") + 
     labs(shape = "") + ylab("p(correct state inference)") + 
...
p_estimation_RT = (df_rule_hit %>% 
  ...
  save_svg_figure("estimation RT plot", 
                  ...
                  unit = "mm") # Supplementary Figure 3A
...
p_distance_true = (df_rule_hit %>% 
    ...
    geom_density() + theme_fig + 
      scale_x_log10() + 
    xlab("distance from the centre of the moles") + labs(color = true_rule_name)) %T>% 
...
p_hit_location_scatter_score =  (df_rule_hit %>% mutate(DisplayScore = numeric_score_to_strings(DisplayScore)) %>% ggplot(aes(x = LocX, LocY, color = DisplayScore)) + geom_point() + theme_fig+ theme_fig + theme(aspect.ratio = 1)+ 
    xlab("X") + ylab("Y") + labs(color = "score") ) %T>% save_svg_figure("hit location scatter plot score", scaling = fig_anova_scale, unit = "mm")
...
p_distance_score = (df_rule_hit %>% 
    ...
    geom_density() + theme_fig +
      scale_x_log10() +
    xlab("distance from the centre of the moles") + labs(color = "score") ) %T>% 
  save_svg_figure("distancescore plot", 
                  width = fig_timeseries_width,
                  height = fig_timeseries_height,
                  scaling = fig_anova_scale, unit = "mm") # Supplementary Figure 3B
...
p_score_ratio_truerule =  (df_rule_hit %>% 
  ...
  save_svg_figure(paste0("ratio of scores for ", true_rule_name), 
                  ...
                  scaling = fig_anova_scale, width = fig_2box_width, height = fig_2box_height, unit = "mm") # Supplementary Figure 2
...
df_rule_hit %>% 
  ...
  pivot_wider(names_from = DisplayScore, values_from = count) %>% 
ungroup() %>% 
mutate(ratio = positive / (positive + negative)) %>% select(PlayerID, TrueRule, ratio) %>% 
...
p_score_ratio_estimation =  (df_rule_hit %>% 
   ...
   coord_cartesian(ylim = c(0.17, 1)) + 
   theme_fig_boxplot + 
   guides(color = guide_legend(color = FALSE)) + xlab(est_rule_name) + ylab("ratio of positive scores")) %T>%
...
df_rule_hit %>% 
  ...
  pivot_wider(names_from = DisplayScore, values_from = count) %>% 
ungroup() %>% 
mutate(ratio = positive / (positive + negative)) %>% select(PlayerID, EstRule, ratio) %>% 
...
p_choice_ratio = df_rule_hit %>% 
    ...
    summarise(count = n()) %>% pivot_wider(names_from = EstRule, values_from = count) %>% mutate(ratio = skill/(skill + random), total = skill + random) %>% ggplot(aes(x = DisplayScore, y = ratio, alpha = DisplayScore, fill = "black")) + gg_filled_box_sina(box_width = box_width_2box) + 
    ...
    theme_fig + guides(color = guide_legend(color = FALSE), alpha=FALSE) + xlab("score") + ylab("P(skill)") 
...
wilcoxsign_choice_ratio = 
  ...
  group_by(PlayerID, DisplayScore, EstRule) %>% 
  summarise(count = n()) %>% 
  pivot_wider(names_from = EstRule, values_from = count) %>% mutate(ratio = skill/(skill + random), total = skill + random) %>% ungroup() %>% 
  select(PlayerID, DisplayScore, ratio) %>% pivot_wider(names_from = DisplayScore, values_from = ratio ) %>% 
...
p_example2 %T>%  save_svg_figure("p_example2", scaling = fig_anova_scale, 
...
## p_val_timeseries---------------------------------------------
df_acc_timeseries_p = 
  df_rule_hit %>% process_for_switch(Correct, accuracy) %>% 
  ungroup() %>% filter(`num of trials` > -1 & `num of trials` < 8) %>% 
...
df_conf_timeseries_p = 
  df_rule_hit %>% process_for_switch(zConfidence, mean_conf) %>% 
  ungroup() %>% filter(`num of trials` > -1 & `num of trials` < 8) %>% 
  group_by(`num of trials`) %>% 
...
p_switch_acc = df_rule_hit %>%
  ...
  stat_summary(aes(linetype=switch), fun.data = mean_se,
               geom = "errorbar",
               width = 0.7,
               size = 0.8, 
               alpha = 0.8,
...
p_switch_conf = df_rule_hit %>%
  ...
  xlab("trials after the switch") + ylab("confidence") + 
  theme_fig + tick_for_time_series
...
p_switch_conf_15 = df_rule_hit %>%
  ...
  xlab("trials after the switch") + ylab("confidence") + 
  theme_fig + tick_for_time_series_15 + 
...
## -------------------------------------------------------------------
p_switch_distance =  
  df_rule_hit %>% process_for_switch(Distance / threshold, Distance) %>% 
    ...
    stat_summary(fun.data = mean_se,
                 ...
                 position=position_dodge(width=0.2)) +
    geom_vline(xintercept = 1 -0.5, linetype = "dashed", color = "gray") +
    coord_cartesian(xlim=c(-3.4, 8.4),ylim = c(0, 2)) + geom_hline(yintercept = 1)+
    xlab("trials after the switch") +
  ylab("mean of normalised distance") + theme_fig + tick_for_time_series
...
p_switch_score = 
  ...
  xlab("trials after the switch") +
  ylab("mean of score\n(0: negative, 1: positive)") + theme_fig + tick_for_time_series
...
df_per_score_test_against_chance_coin = 
  ...
  mutate(
    wilcoxsign = 
      map(.x = data, .f = 
            ~.x %>% select(accuracy, chance_level) %>%
...
p_conf_score = 
  df_rule_hit_for_test %>% 
  group_by(PlayerID, DisplayScore) %>% summarise(zConfidence = mean(zConfidence)) %>% 
...
wilcoxsign_conf_correct = 
  ...
  mutate(
    Correct = if_else(Correct, "correct", "incorrect")) %>% 
  group_by(PlayerID, Correct) %>% 
  summarise(zConfidence = mean(zConfidence)) %>% ungroup() %>% 
...
## -------------------------------------------------------------------
df_rule_hit_for_test %>% 
    summarize_performance(DisplayScore, Correct, TrueRule) %>% select(PlayerID, DisplayScore, TrueRule, mean_Correct) %>% anovakun("sAB", long = TRUE, gg = TRUE) %>%
  sink_analysis(filename = "anova_DisplayScore_TrueRule_mean_Correct.txt", analysis_group = "accuracy_score_truerule")

df_rule_hit_for_test %>% 
  lme4::glmer(Correct ~ TrueRule * DisplayScore + (1 | PlayerID), data = ., family = binomial) %>% output_lme_results("lme_accuracy_score_truerule",
                                                                                                                      analysis_group = "accuracy_score_truerule")

df_rule_hit_for_test %>% 
    summarize_performance(DisplayScore, Correct, TrueRule) %>% select(PlayerID, DisplayScore, TrueRule, mean_Correct) %>% 
...
df_rule_hit_for_test %>% 
    summarize_performance(DisplayScore, Correct, EstRule) %>% select(PlayerID, DisplayScore, EstRule, mean_Correct) %>% anovakun("sAB", long = TRUE, gg = TRUE) %>% 
...
df_rule_hit_for_test %>% 
    summarize_performance(DisplayScore, Correct, EstRule) %>% select(PlayerID, DisplayScore, EstRule, mean_Correct) %>%
...
## -------------------------------------------------------------------
df_rule_hit_for_test %>% 
    summarize_performance(DisplayScore, zConfidence, TrueRule) %>% select(PlayerID, DisplayScore, TrueRule, mean_zConfidence) %>% anovakun("sAB", long = TRUE, gg = TRUE) %>% 
...
## -------------------------------------------------------------------
df_rule_hit_for_test %>% 
    summarize_performance(DisplayScore, zConfidence, EstRule) %>% select(PlayerID, DisplayScore, EstRule, mean_zConfidence) %>% anovakun("sAB", long = TRUE, gg = TRUE) %>% 
...
## -------------------------------------------------------------------
df_rule_hit_for_test %>% 
    summarize_performance(DisplayScore, zConfidence, EstRule) %>% select(PlayerID, DisplayScore, EstRule, mean_zConfidence) %>% aov(mean_zConfidence ~ DisplayScore*EstRule + Error(PlayerID/(DisplayScore*EstRule)), data = .) %>% summary() # almost same result as anova-kun
...
p_switch_prev_choice_score = 
  ...
  gg_filled_box_sina() + 
  theme_fig + ylab("switch probability") + xlab("previouce choice") +
...
p_conf_switch = df_rule_hit %>% 
  ...
  gg_filled_box_sina() + 
  coord_cartesian(ylim = c(0, 1)) + 
  theme_fig + ylab("switch probability") + xlab("confidence") +
  labs(alpha = "score") + 
  theme(legend.position = "right") + 
  theme(axis.line.x = element_blank(), axis.ticks.x = element_blank()) + 
  guides(alpha=guide_legend(override.aes = list(color="black", fill = c("black"))), fill = FALSE)
p_conf_switch %>% save_svg_figure("p_conf_switch", scaling = fig_anova_scale, width = fig_anova_width, height = fig_anova_height, unit = "mm",
                                  analysis_group = "switch_conf_score")

(p_conf_switch + coord_cartesian(ylim = c(0, 1))) %>% save_svg_figure("p_conf_switch_axis_scaled", scaling = fig_anova_scale, width = fig_anova_width, height = fig_anova_height, unit = "mm", 
                                                                      analysis_group = "switch_conf_score")
...
(df_rule_hit_for_test %>%
  process_for_summary_anova() %>% group_by(PlayerID, conf_binary, TrueRule) %>% summarise(switch = mean(switch)) %>% 
    ggplot(aes(x = conf_binary, y = switch, group = interaction(conf_binary, TrueRule), color = TrueRule)) + geom_boxplot(outlier.shape = NA) + geom_sina_fitted() + theme_fig + ylab("switch probability") + xlab("confidence") + labs(color = "true state")) %>% 
...
df_rule_hit %>% 
  process_for_summary_anova() %>% 
  do_anova(prev_choice, DisplayScore, switch) %>% print_anova_sentences()
...
p_distance_and_choice_conf = df_rule_hit %>% 
  ...
  geom_line(stat = "smooth", aes(group=PlayerID, alpha = 0.2), alpha = 0.2, method = "glm", method.args = list(family = binomial(link="logit")), se=FALSE) + xlab("normalised distance") +
  labs(color = "confidence level") + 
  facet_grid(.~DisplayScore) + theme_fig + scale_color_hue()
...
p_choice_distance_score = (df_rule_hit %>% 
  ...
  geom_line(stat = "smooth", aes(group=interaction(PlayerID, DisplayScore), alpha = 0.2), alpha = 0.2, method = "glm", method.args = list(family = binomial(link="logit")), se=FALSE)  + xlab("normalised distance") + theme_fig + labs(color = "score") ) %T>% 
...
# p_distance_and_accuracy = 
  ...
  geom_line(
    stat = "smooth", 
    aes(group = interaction(PlayerID, DisplayScore), 
        alpha = 0.2), 
    alpha = 0.2, 
    method = "glm", 
    method.args = list(family = binomial(link = "logit")), 
    se = FALSE) + 
  xlab("normalised distance") + ylab("p(correct state inference)") + theme_fig + labs(color = "score")
...
# p_distance_and_accuracy_true = 
  ...
  geom_line(
    stat = "smooth", 
    aes(group = interaction(PlayerID, TrueRule), 
        alpha = 0.2), 
    alpha = 0.2, 
    method = "glm", 
    method.args = list(family = binomial(link = "logit")), 
    se = FALSE) + 
  xlab("normalised distance") + ylab("p(correct state inference)") + theme_fig + labs(color = "task state")
...
# p_distance_and_accuracy_score = 
  ...
  geom_line(
    ...
    method.args = list(family = binomial(link = "logit")), 
    se = FALSE) + 
  xlab("normalised distance") + ylab("p(correct state inference)") + theme_fig + labs(color = "score") + facet_wrap(~TrueRule)
...
p_prev_estimation_distance = 
  df_rule_hit %>% 
  group_by(PlayerID) %>% 
  mutate(prev_choice = lag(EstRule), 
         DisplayScore = numeric_score_to_strings(DisplayScore), 
         conf_binary = if_else(zConfidence > 0, "high", "low"),
         switch = prev_choice != EstRule,
         scaledDistance = Distance / threshold) %>% 
  ungroup() %>% group_by(PlayerID, prev_choice) %>% 
  summarize(scaledDistance = mean(scaledDistance)) %>% drop_na() %>%  
  ...
  theme_fig + coord_cartesian(ylim=c(0, 5)) + 
  xlab("previous estimation") + ylab("normalised distance")
...
p_distance_diff_prev = 
  df_distance%>% 
    ...
    ylab(expression(paste({`[distance]`[t]}, " - ", {`[distance]`[t -1]}))) + theme_fig + theme(legend.position = "right") +
    labs(alpha = expression(paste({`[score]`[t]})), fill = "") + guides(fill = FALSE) + 
...
## -------------------------------------------------------------------
df_distance%>% 
  drop_na(prev_distance) %>% select(-next_distance) %>% 
...
## -------------------------------------------------------------------
df_distance%>% 
  drop_na(next_distance) %>% select(-prev_distance) %>% 
...
lm_diff_conf_dis = function(df, trials){
  ...
  res_all = df2 %>% 
    ...
    tidy() %>% mutate(formula = "diff_dis ~ diff_conf * switch")
  res_post_skill = df2 %>% 
    dplyr::filter(switch == "TrialsAfterSwitchToSkill") %>% 
    estimatr::lm_robust(diff_dis ~ diff_conf, data = .) %>%
    tidy() %>% mutate(formula = "diff_dis ~ diff_conf (random to skill)")
  res_post_random = df2 %>% 
    dplyr::filter(switch == "TrialsAfterSwitchToRandom") %>% 
    estimatr::lm_robust(diff_dis ~ diff_conf, data = .) %>%
    tidy() %>% mutate(formula = "diff_dis ~ diff_conf (skill to random)")
...
lm_diff_conf_dis_abs = function(df, trials){
  ...
  res_all = df2 %>% 
    ...
    tidy() %>% mutate(formula = "abs(diff_dis) ~ abs(diff_conf) * switch")
  res_post_skill = df2 %>% 
    dplyr::filter(switch == "TrialsAfterSwitchToSkill") %>% 
    estimatr::lm_robust(abs(diff_dis) ~ abs(diff_conf), data = .) %>%
    tidy() %>% mutate(formula = "abs(diff_dis) ~ abs(diff_conf) (random to skill)")
  res_post_random = df2 %>% 
    dplyr::filter(switch == "TrialsAfterSwitchToRandom") %>% 
    estimatr::lm_robust(abs(diff_dis) ~ abs(diff_conf), data = .) %>%
    tidy() %>% mutate(formula = "abs(diff_dis) ~ abs(diff_conf) (skill to random)")
  rbind(res_all, res_post_skill, res_post_random)%>% 
    select(formula, term, estimate, std.error, statistic, p.value, df)
}
...
ggplot(aes(x = mean_diff_conf %>% abs(), y = mean_diff_dis %>% abs(), color = switch))   + geom_point()+ stat_smooth(method = estimatr::lm_robust) + stat_smooth(aes(group = 1), color = "black", method = estimatr::lm_robust) + facet_wrap(.~Trials) +  theme_fig + xlab("|mean difference of confidence|") + ylab("|mean difference of distance|")) %>% save_svg_figure("conf_dis_diff_mean_abs", scaling = fig_anova_scale, width = fig_anova_width, height = fig_anova_height, unit = "mm", analysis_group = "conf_dis_diff")
...
(df_rule_hit %>% 
  ...
  mutate(switch = 
           ...
           str_replace_all("TrialsAfterSwitchToSkill", "random to skill")) %>% 
  mutate(Trials = label_conf_dis_diff[Trials] %>% unlist()) %>% 
ggplot(aes(x = mean_diff_conf, y = mean_diff_dis, color = switch))   + geom_point()+ stat_smooth(method = estimatr::lm_robust) + stat_smooth(aes(group = 1), color = "black", method = estimatr::lm_robust) + facet_wrap(.~Trials) +  theme_fig + xlab("mean difference of confidence") + ylab("mean difference of distance") )%>% save_svg_figure("conf_dis_diff_mean", scaling = fig_anova_scale, width = fig_anova_width, height = fig_anova_height, unit = "mm", analysis_group = "conf_dis_diff")
...
(df_rule_hit %>% 
  ...
  mutate(switch = 
           ...
           str_replace_all("TrialsAfterSwitchToSkill", "random to skill")) %>% 
  mutate(Trials = label_conf_dis_diff[Trials] %>% unlist()) %>% 
  ggplot(aes(x = diff_conf %>% abs() , y = diff_dis %>% abs(), color = switch))   + geom_point() + stat_smooth(method = estimatr::lm_robust) + stat_smooth(aes(group = 1), color = "black", method = estimatr::lm_robust) + facet_wrap(.~Trials) + theme_fig1 + 
  xlab("|difference of confidence|") + ylab("|difference of distance|")) %>% save_svg_figure("conf_dis_diff_abs", scaling = fig_anova_scale, width = fig_anova_width, height = fig_anova_height, unit = "mm", analysis_group = "conf_dis_diff")
...
(df_rule_hit %>% 
  ...
  mutate(switch = 
           ...
           str_replace_all("TrialsAfterSwitchToSkill", "random to skill")) %>% 
  mutate(Trials = label_conf_dis_diff[Trials] %>% unlist()) %>% 
  ggplot(aes(x = diff_conf, y = diff_dis, color = switch))   + geom_point() + stat_smooth(method = estimatr::lm_robust) + stat_smooth(aes(group = 1), color = "black", method = estimatr::lm_robust) + facet_wrap(.~Trials) + theme_fig1 + 
  xlab("difference of confidence") + ylab("difference of distance")) %>% save_svg_figure("conf_dis_diff", scaling = fig_anova_scale, width = fig_anova_width, height = fig_anova_height, unit = "mm", analysis_group = "conf_dis_diff")
...
df_rule_hit %>% 
    ...
    mutate(switch = 
               str_replace_all(switch, "TrialsAfterSwitchToRandom", "skill to random") %>% 
               str_replace_all("TrialsAfterSwitchToSkill", "random to skill")) %>% 
    mutate(Trials = label_conf_dis_diff[Trials] %>% unlist()) %>% group_by(Trials) %>% nest() %>% tibble_lme(diff_dis ~ diff_conf * switch + (1|PlayerID)) %>% unnest(cols=c(res)) %>% select(-data) %>% 
  rbind(
    df_rule_hit %>% 
        calc_conf_dis_difference() %>% 
        filter(Trials < 3 & Trials >= 1) %>% 
...
(df_score_hit %>%
  ...
  inner_join(df_rule_hit %>% 
               ...
               summarise(true_threshold = unique(true_threshold)), 
             by ="PlayerID") %>% 
  ggplot(aes(x = true_threshold, y = error)) + geom_point() + 
...
df_score_hit %>%
  ...
  inner_join(df_rule_hit %>% 
               ...
               summarise(true_threshold = unique(true_threshold)), 
             by ="PlayerID") %>% 
             lm_robust(error ~ true_threshold, data = .) %>% summary() %>% print() %>% 
  sink_analysis(filename = "lm_robust_score_estimate_error_true_threshold.txt", 
                analysis_group = "scaled_score_estimate_error_true_threshold")
...
# output the result of the robust regression of the fig above to a file
df_score_conf_lme %>% 
  lm_robust(estimate ~ true_threshold, data = .) %>% summary() %>% print() %>% 
  sink_analysis(filename = "lm_robust_coeff_true_threshold.txt", 
                analysis_group = "coeff_true_threshold")
...
(df_rule_hit %>% 
  ...
  geom_line() + 
  geom_boxplot(aes(group = EstRuleConfidence)) + 
  theme_fig + labs(subtitle = "true state") + 
  facet_wrap(.~TrueRule)) %>% 
...
(df_rule_hit %>% 
  ...
  geom_line() + 
  geom_boxplot(aes(group = EstRuleConfidence)) + 
  theme_fig + labs(subtitle = "choice") + 
